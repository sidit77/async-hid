...
mod tests {
    ...
    fn test_try_flatten_iter() {
        let iter = TryFlattenIter::<_, Vec<i32>> {
            inner: [Err(0), Ok(vec![1, 2, 3]), Err(4), Ok(vec![5, 6, 7]), Err(8), Err(9)].into_iter().fuse(),
            current: None,
        };
        assert_eq!(iter.collect::<Vec<_>>(), vec![Err(0), Ok(1), Ok(2), Ok(3), Err(4), Ok(5), Ok(6), Ok(7), Err(8), Err(9)]);
    }
